...
const (
	collectionName = "user_resets"
)

type ResetsService struct {
...
func New(services *structures.RequestValues) *ResetsService {
	s := &ResetsService{}
	s.services = services
...
func (s *ResetsService) List(filter bson.D) (*models.UserResetsModel, *logger.HttpError) {
...
func (s *ResetsService) Get(filter bson.D, model *models.UserResetModel) *logger.HttpError {
...
func (s *ResetsService) Create(model *models.UserResetModel) *logger.HttpError {
...
func (s *ResetsService) Update(model *models.UserResetModel) *logger.HttpError {

	if err := s.Validate(model); err != nil {
		return err
	}

	exists := &models.UserResetModel{}
...
func (s *ResetsService) Delete(model *models.UserResetModel) *logger.HttpError {
...
func (m *ResetsService) Validate(model *models.UserResetModel) *logger.HttpError {

	validate := validator.New()

	if err := validate.Var(model.UserResetTypeID, "required"); err != nil {
		fields := []string{"userResetTypeId"}
		return logger.Error(logger.LogStatusBadRequest, &fields, "invalid userResetTypeId ", err, nil)
	}
	...
	if err := validate.Var(model.ResetKey, "required,gte=1"); err != nil {
		fields := []string{"resetKey"}
		return logger.Error(logger.LogStatusBadRequest, &fields, "invalid resetKey ", err, nil)
	}

	if err := validate.Var(model.Expire, "required,gte=1"); err != nil {
		fields := []string{"Expire"}
		return logger.Error(logger.LogStatusBadRequest, &fields, "invalid Expire ", err, nil)
	}
...
func (s *ResetsService) assign(to *models.UserResetModel, from *models.UserResetModel, operation services.Operation) {
	...
	if operation == services.SVC_OPERATION_CREATE {
		...
		to.UserResetTypeID = from.UserResetTypeID
		to.UserUserID = from.UserUserID
		to.ResetKey = from.ResetKey
		to.Used = from.Used
		to.Expire = from.Expire
	}
