...
func New(services *structures.RequestValues) *RolesService {
	s := &RolesService{}
	s.services = services
...
func (s *RolesService) List(filter bson.D) (*models.UserRolesModel, *logger.HttpError) {
...
func (s *RolesService) Get(filter bson.D, model *models.UserRoleModel) *logger.HttpError {
...
func (s *RolesService) Create(model *models.UserRoleModel) *logger.HttpError {

	if err := s.Validate(model); err != nil {
		return err
	}

	exists := &models.UserRoleModel{}
	if err := s.Get(
...
func (s *RolesService) Update(model *models.UserRoleModel) *logger.HttpError {
...
func (s *RolesService) Delete(model *models.UserRoleModel) *logger.HttpError {
...
func (m *RolesService) Validate(model *models.UserRoleModel) *logger.HttpError {

	validate := validator.New()

	if err := validate.Var(model.Name, "required"); err != nil {
		fields := []string{"name"}
		return logger.Error(logger.LogStatusBadRequest, &fields, "invalid name ", err, nil)
	}

	if err := validate.Var(model.GroupPath, "required"); err != nil {
		fields := []string{"groupPath"}
		return logger.Error(logger.LogStatusBadRequest, &fields, "invalid groupPath ", err, nil)
	}

	return nil
}

func (s *RolesService) assign(to *models.UserRoleModel, from *models.UserRoleModel, operation services.Operation) {
